...
def handler(event: dict[str, Any], _context: Any) -> dict[str, Any]:
    ...
    try:
        ddb.update_item(
            ...
            Key={"pk": {"S": _pk(workflow_id)}, "sk": {"S": _sk_task(task_id)}},
    ...
    try:
        resp = lambda_client.invoke(
            ...
            Payload=json.dumps({"workflowId": workflow_id, "taskId": task_id}).encode("utf-8"),
        )
    ...
    try:
        ...
        ddb.update_item(
            TableName=TABLE_NAME,
            Key={"pk": {"S": _pk(req["workflowId"])}, "sk"
            : {"S": _sk_task(req["taskId"]) }},
            UpdateExpression="SET #s = :succeeded, #res = :res, durationMs = :dur",
            ExpressionAttributeNames={"#s": "status", "#res": "result"},
            ExpressionAttributeValues={
                ":succeeded": {"S": "SUCCEEDED"},
